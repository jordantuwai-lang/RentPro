...
export default function PartnersPage() {
  ...
  const [tab, setTab] = useState<'repairers' | 'insurers' | 'tow-operators'>('repairers');
  ...
  const tabs = [
    { key: 'repairers', label: 'Repairers' },
    { key: 'insurers', label: 'Insurers' },
    { key: 'tow-operators', label: 'Tow Operators' },
  ] as const;
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <div>
          ...
          <p style={{ color: '#64748b', fontSize: '14px', marginTop: '4px' }}>Repairers, insurers and tow operator directory</p>
        </div>
        {isAdmin && (
          <div style={{ display: 'flex', gap: '8px' }}>
            {tab === 'repairers' && (
              <>
                ...
                <button onClick={() => {}} style={{ background: '#01ae42', color: '#fff', padding: '10px 16px', borderRadius: '8px', border: 'none', fontSize: '14px', fontWeight: '500', cursor: 'pointer' }}>
                  + Bulk Add
                </button>
              </>
            )}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid #e2e8f0', background: '#f8fafc' }}>
                {['Name', 'Address', 'Phone', 'Email', 'Suburb', 'State', 'Sales Rep'].map(h => (
            ...
            <tbody>
              {loadingRepairers ? (
                ...
                <tr><td colSpan={7} style={{ padding: '40px', textAlign: 'center', color: '#94a3b8' }}>No repairers yet.</td></tr>
              ) : repairers?.map((r: any) => (
                <tr key={r.id} onClick={() => window.location.href = `/dashboard/partners/repairer/${r.id}`} style={{ borderBottom: '1px solid #f1f5f9', cursor: 'pointer' }}>
                  ...
                  <td style={{ padding: '14px 16px', fontSize: '14px', color: '#64748b' }}>{r.address || '—'}</td>
      ...
      {tab === 'tow-operators' && (
        <div style={{ background: '#fff', borderRadius: '12px', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid #e2e8f0', background: '#f8fafc' }}>
                {['Name', 'Address', 'Phone', 'Email', 'Suburb', 'State'].map(h => (
                  <th key={h} style={{ padding: '12px 16px', textAlign: 'left', fontSize: '12px', fontWeight: '600', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              <tr>
                <td colSpan={6} style={{ padding: '40px', textAlign: 'center', color: '#94a3b8' }}>No tow operators yet.</td>
              </tr>
            </tbody>
          </table>
        </div>
