  ...
  return (
    <div>
          ...
          <div style={{ maxWidth: '700px', margin: '0 auto' }}>
            ...
            <div style={section}>
              ...
              <div style={grid2}>
                ...
                <Field label="Vehicle" value={selectedFile.vehicle ? `${selectedFile.vehicle.make} ${selectedFile.vehicle.model} · ${selectedFile.vehicle.registration}` : undefined} />
              </div>
            </div>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200, padding: '20px' }}>
          <div style={{ background: '#fff', borderRadius: '16px', padding: '32px', width: '100%', maxWidth: '500px', maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
              ...
              <button onClick={() => setShowReturnModal(false)} style={{ background: 'none', border: 'none', fontSize: '22px', cursor: 'pointer', color: '#64748b' }}>x</button>
            </div>
      ...
      <div style={{ marginBottom: '24px' }}>
        ...
        <p style={{ color: '#64748b', fontSize: '14px', marginTop: '4px' }}>Active hire files — {selectedBranch?.name || 'All branches'}</p>
      </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ borderBottom: '1px solid #e2e8f0', background: '#f8fafc' }}>
              {['File #', 'Rez #', 'Customer', 'Phone', 'Vehicle', 'Branch', 'Start Date', 'Days on Hire'].map(h => (
          ...
          <tbody>
            {isLoading ? (
              ...
              <tr><td colSpan={8} style={{ padding: '40px', textAlign: 'center', color: '#94a3b8' }}>No active hire files.</td></tr>
            ) : reservations?.map((r: any) => (
              <tr key={r.id} onClick={() => setSelectedFile(r)} style={{ borderBottom: '1px solid #f1f5f9', cursor: 'pointer' }}>
                <td style={{ padding: '14px 16px', fontSize: '13px', fontWeight: 700, color: '#01ae42' }}>{r.fileNumber || '—'}</td>
                <td style={{ padding: '14px 16px', fontSize: '13px', color: '#64748b' }}>{r.reservationNumber}</td>
                ...
                <td style={{ padding: '14px 16px', fontSize: '13px', color: '#0f172a' }}>{r.vehicle?.make} {r.vehicle?.model} · {r.vehicle?.registration}</td>
                <td style={{ padding: '14px 16px', fontSize: '13px', color: '#64748b' }}>{r.vehicle?.branch?.code}</td>
