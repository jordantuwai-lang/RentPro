#!/bin/bash
set -e
REPO="$HOME/rentpro/apps/api/src/users"
echo "Writing GPS tracking files..."

echo "  -> users.service.ts"
cat > "$REPO/users.service.ts" << 'HEREDOC_USERS_SERVICE_TS'
...
HEREDOC_USERS_SERVICE_TS

echo "  -> users.controller.ts"
cat > "$REPO/users.controller.ts" << 'HEREDOC_USERS_CONTROLLER_TS'
...
HEREDOC_USERS_CONTROLLER_TS

echo "  -> map page"
mkdir -p "$HOME/rentpro/apps/web/src/app/dashboard/logistics/map"
cat > "$HOME/rentpro/apps/web/src/app/dashboard/logistics/map/page.tsx" << 'HEREDOC_MAP_PAGE_TSX'
'use client';
